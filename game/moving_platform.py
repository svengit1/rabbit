...
class MovingPlatform():
    ...
    def create(self, **kwargs):
        ...
        self.content = [
            self.get_sprite(map_images[i[0]], i[1], y)
            for i in [(217, x), (218, x + 32)]
        ]

        self.content += [
            self.get_sprite(self.middle, x + 32 * 2 + i * 32, y)
            for i in range(0, kwargs['size'] - 4)
        ]

        self.content += [
            self.get_sprite(map_images[i[0]], i[1], y)
            for i in [(223, x + (kwargs['size'] - 2) *
                       32), (224, x + (kwargs['size'] - 2) * 32 + 32)]
        ]
        static_line = pymunk.Segment(self.space.static_body, (x + 32, y + 32),
                                     (x + 32 +
                                      (kwargs['size'] - 2) * 32, y + 32), 0.0)
